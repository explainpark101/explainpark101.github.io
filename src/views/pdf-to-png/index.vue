  ...
  <div class="min-h-screen bg-slate-50 text-slate-900">
    ...
    <div class="max-w-4xl mx-auto py-16 px-6">
      <header class="text-center mb-16">
        <div
          class="inline-flex items-center justify-center p-3 bg-indigo-600 rounded-2xl shadow-lg shadow-indigo-500/20 mb-2 text-white">
        ...
        <h1 class="text-4xl font-black tracking-tight m-0 mb-4">PDF to PNG <span
            class="text-indigo-600">Converter</span></h1>
      ...
      <main class="bg-white rounded-3xl shadow-2xl p-8 border border-slate-100">
        <div class="border-2 border-dashed rounded-2xl p-16 text-center cursor-pointer transition-all"
          ...
          <div class="flex flex-col items-center gap-4">
            <div class="p-4 rounded-full transition-transform"
        ...
        <div v-if="error"
          class="flex items-center gap-2 mt-6 p-4 bg-red-50 text-red-600 rounded-xl border border-red-100">
        ...
        <div v-if="isConverting || progress > 0" class="mt-10">
          <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-3">
              <svg v-if="isConverting" class="w-5 h-5 animate-spin" width="20" height="20" viewBox="0 0 24 24"
          ...
          <div class="h-3 bg-slate-100 rounded-full overflow-hidden">
            <div class="h-full bg-indigo-600 transition-all duration-300 ease-out" :style="{ width: progress + '%' }">
            </div>
          </div>
        </div>
        ...
        <div v-if="convertedImages.length > 0" class="mt-16">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            <div v-for="img in convertedImages" :key="img.id"
              ...
              <div
                class="absolute inset-0 bg-slate-900/60 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center gap-3 transition-opacity pointer-events-none group-hover:pointer-events-auto">
                <div class="bg-white/20 backdrop-blur-xl px-4 py-1.5 rounded-full text-white font-bold text-sm">Page {{
                  img.id }}</div>
                <div class="flex flex-col gap-2 w-[min(100%,220px)] px-2">
                  <button type="button" @click.stop="downloadSingle(img.url, img.name)"
                  ...
                  <button type="button" @click.stop="copyImageToClipboard(img.url)"
                  ...
                  <button type="button" @click.stop="openPreviewModal(img)"
                    class="flex items-center justify-center gap-2 bg-white/95 text-slate-800 py-2 px-4 rounded-xl font-bold border-none cursor-pointer shadow-lg transition-all hover:bg-white active:scale-[0.98] text-sm">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                  ...
                  <button type="button" @click.stop="openPreviewNewTab(img.url)"
                    class="flex items-center justify-center gap-2 bg-white/95 text-slate-800 py-2 px-4 rounded-xl font-bold border-none cursor-pointer shadow-lg transition-all hover:bg-white active:scale-[0.98] text-sm">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  ...
                  <button type="button" @click.stop="sendImageToGemini(img)"
                    class="flex items-center justify-center gap-2 bg-white text-emerald-700 py-2 px-4 rounded-xl font-bold border-none cursor-pointer shadow-lg transition-all hover:bg-emerald-50 active:scale-[0.98] text-sm">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              ...
              <div class="p-3 bg-slate-50/50 border-t border-slate-100">
                <p
                  class="text-xs font-medium text-slate-500 m-0 mb-2 overflow-hidden text-ellipsis whitespace-nowrap px-1 text-center">
                  {{ img.name }}</p>
                <div class="flex flex-wrap items-center justify-center gap-1.5">
                  <button type="button" @click="downloadSingle(img.url, img.name)"
                  ...
                  <button type="button" @click="copyImageToClipboard(img.url)"
                  ...
                  <button type="button" @click="openPreviewModal(img)"
                    class="inline-flex items-center gap-1 rounded-lg bg-slate-200 text-slate-800 text-[11px] font-bold px-2.5 py-1.5 border-none cursor-pointer hover:bg-slate-300"
                    title="크게 보기">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                      stroke-linecap="round" stroke-linejoin="round">
                  ...
                  <button type="button" @click="openPreviewNewTab(img.url)"
                    class="inline-flex items-center gap-1 rounded-lg bg-slate-200 text-slate-800 text-[11px] font-bold px-2.5 py-1.5 border-none cursor-pointer hover:bg-slate-300"
                    title="새 탭에서 미리보기">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                      <path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" />
                  ...
                  <button type="button" @click="sendImageToGemini(img)"
                    class="inline-flex items-center gap-1 rounded-lg bg-emerald-600 text-white text-[11px] font-bold px-2.5 py-1.5 border-none cursor-pointer hover:bg-emerald-700"
                    title="Gemini로 보내기">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                      <path d="M21 12a9 9 0 1 1-6.22-8.56" />
                      <polyline points="22 4 12 14.01 9 11.01" />
    ...
    <Teleport to="body">
      <div v-if="previewModal"
        class="fixed inset-0 z-[2000] overflow-y-auto bg-slate-950/85 p-2 sm:p-4 backdrop-blur-sm" role="dialog"
        aria-modal="true" aria-label="이미지 크게 보기" @click.self="closePreviewModal">
        <div class="flex min-h-full items-center justify-center">
          <div class="relative my-2 flex w-full max-w-[96vw] flex-col items-center gap-3">
            <button type="button"
              ...
              aria-label="닫기" @click="closePreviewModal">
            ...
            <div class="relative flex w-full items-center justify-center px-11 sm:px-14">
              <button type="button"
                ...
                :disabled="!canGoPrevPage" aria-label="이전 페이지 (←)" @click="goToPreviousPage">
                ←
              </button>
              <img :src="previewModal.url" :alt="previewModal.alt"
              ...
              <button type="button"
                ...
                :disabled="!canGoNextPage" aria-label="다음 페이지 (→)" @click="goToNextPage">
                →
              </button>
            </div>
...
function sendImageToGemini(img) {
  window.postMessage(
    {
      code: 'P2G_IMAGE_V1',
      dataUrl: img.url,
    ...
    // 'https://explainpark101.github.io',
    'http://localhost:5173',
  );

  copyToast.value = 'Gemini로 전송 요청을 보냈습니다.';
  if (copyToastTimer) {
    clearTimeout(copyToastTimer);
  }
  copyToastTimer = setTimeout(() => {
